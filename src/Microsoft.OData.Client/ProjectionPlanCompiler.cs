    ...
    internal class ProjectionPlanCompiler : ALinqExpressionVisitor
    {
        ...
        internal override Expression VisitConditional(ConditionalExpression conditional)
        {
            ...
            if (!nullCheck.Match || !ClientTypeUtil.TypeOrElementTypeIsEntity(ResourceBinder.StripConvertToAssignable(nullCheck.TestToNullExpression).Type))
            {
                ...
                if (nullCheck.Match)
                {
                    // Suppose we are processing the following query and Vacations is a collectionComplexType property:
                    // c.CreateQuery<Person>("People").Select(
                    //     e => new MyEmployeeEntity
                    //         {
                    //             ID = e.ID,
                    //             Vacations = (e as Manager).Vacations == null ? null : ...
                    //
                    // The expression "(e as Manager).Vacations" will get translated to "Convert(ProjectionValueForPath((e as Manager)->.Vacations), ICollection<Vacation>)".
                    //
                    // During materialization for a collection property in ProjectionValueForPath(), we add elements of the collection to the
                    // target object (i.e. MyEmployeeEntity.Vacations) and return the target object instead of activating a new instance of Manager.Vacations.
                    // However if MyEmployeeEntity.Vacations is of type ICollection<MyVacationComplexType>, ProjectionValueForPath((e as Manager)->.Vacations) actually returns
                    // an instance of ICollection<MyVacationComplexType> and thus converting it to ICollection<Vacation> will result in an InvalidCastException.
                    //
                    // Since this is a null check and the type conversion serves no real purpose, we are removing the covert from testToNullExpression.
                    Expression testToNullExpression = this.Visit(nullCheck.TestToNullExpression);
                    if (testToNullExpression.NodeType == ExpressionType.Convert)
                    {
                        testToNullExpression = ((UnaryExpression)testToNullExpression).Operand;
                    }

                    test = Expression.MakeBinary(ExpressionType.Equal, testToNullExpression, Expression.Constant(null));
                }
        ...
        private Expression RebindNewExpressionForDataServiceCollectionOfT(NewExpression nex)
        {
            ...
            NewExpression visited = this.VisitNew(nex);
            Expression rebound = null;
            ExpressionAnnotation annotation = null;

            if (visited != null)
            {
                ConstructorInfo constructorInfo =
                    nex.Type.GetInstanceConstructors(false /*isPublic*/).First(
                ...
                if (visited.Arguments.Count == 1 && visited.Constructor == nex.Type.GetInstanceConstructor(true /*isPublic*/, new[] { enumerable }) &&
                ...
                else if (visited.Arguments.Count == 2 &&
                         this.annotations.TryGetValue(visited.Arguments[0], out annotation))
                {
                    // DataServiceCollection<T>(IEnumerable<T> items, TrackingMode trackingMode)
                    // ->
                    ...
                    rebound = RebindConstructor(
                        ...
                        this.materializerExpression,
                        Expression.Constant(null, typeof(DataServiceContext)),
                        visited.Arguments[0], // items
                        visited.Arguments[1], // TrackingMode
                        Expression.Constant(null, typeof(string)),
                        Expression.Constant(null, typeof(Func<EntityChangedParams, bool>)),
                        Expression.Constant(null, typeof(Func<EntityCollectionChangedParams, bool>)));
                }
                else if (visited.Arguments.Count == 5 &&
                         this.annotations.TryGetValue(visited.Arguments[0], out annotation))
                {
                    ...
                    rebound = RebindConstructor(
                        ...
                        visited.Arguments[0],   // items
                        visited.Arguments[1],   // TrackingMode
                        visited.Arguments[2],   // entityset name
                        visited.Arguments[3],   // entity changed cb
                        visited.Arguments[4]);  // collection changed cb
                }
                else if (visited.Arguments.Count == 6 &&
                         typeof(DataServiceContext).IsAssignableFrom(visited.Arguments[0].Type) &&
                         this.annotations.TryGetValue(visited.Arguments[1], out annotation))
                {
                    // DataServiceCollection<T>(
                    ...
                    rebound = RebindConstructor(
                        constructorInfo,
                        this.materializerExpression,
                        visited.Arguments[0],   // context
                        visited.Arguments[1],   // items
                        visited.Arguments[2],   // trackingMode
                        visited.Arguments[3],   // entityset name
                        visited.Arguments[4],   // entity changed cb
                        visited.Arguments[5]);  // collection changed cb
                }
            }
        ...
        private Expression RebindMethodCallForMemberSelect(MethodCallExpression call)
        {
            ...
            if (annotation != null)
            {
                ...
                {
                    result = Expression.Call(call.Method, parameterSource, selectorExpression);
                    this.annotations.Add(result, annotation);
                }
            }
        ...
        private Expression TypedEnumerableToList(Expression source, Type targetType)
        {
            ...
            // CONSIDER: change to a generic thing to pull the IEnumerable<T> than just assuming it's always
            // generic (so a property can be of class ClientList : List<Client>).
